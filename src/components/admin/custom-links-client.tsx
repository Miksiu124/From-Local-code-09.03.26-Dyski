...
import { Plus, Edit, Trash2, LineChart, Link as LinkIcon, TrendingUp, RefreshCw, AlertCircle } from "lucide-react";
...
import type { CustomLink as CustomLinkType } from "@/lib/admin-api";

interface CustomLinksClientProps {
    initialLinks: CustomLinkType[];
}

export function CustomLinksClient({ initialLinks }: CustomLinksClientProps) {
    const t = useTranslations("admin");
    const [links, setLinks] = useState<CustomLinkType[]>(initialLinks);
    const [loading, setLoading] = useState(false);
    const [fetchError, setFetchError] = useState<string | null>(null);

    // Create/Edit Dialog
    const [dialogOpen, setDialogOpen] = useState(false);
    const [editingLink, setEditingLink] = useState<CustomLinkType | null>(null);
    ...
    const [selectedLink, setSelectedLink] = useState<CustomLinkType | null>(null);
    ...
    const fetchLinks = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch("/api/admin/custom-links", {
                credentials: "include",
                cache: "no-store",
            });
            if (res.ok) {
                ...
                const msg =
                    res.status === 401
                        ...
                        : res.status === 403
                          ...
                          : `Błąd ładowania (${res.status}). Spróbuj ponownie.`;
                setFetchError(msg);
                logger.error("Failed to fetch custom links", { status: res.status });
            }
        } catch (err) {
            setFetchError("Błąd połączenia. Sprawdź sieć i spróbuj ponownie.");
    ...
    const openEdit = (link: CustomLinkType) => {
        setEditingLink(link);
        setForm({
            slug: link.slug,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (editingLink) {
                    setLinks((prev) =>
                        prev.map((l) => (l.id === editingLink.id ? { ...l, ...data, visitsCount: l.visitsCount, registrationsCount: l.registrationsCount, purchasesCount: l.purchasesCount, revenue: l.revenue } : l))
                    );
                } else if (data?.id) {
                    setLinks((prev) => [
                        {
                ...
                const msg = data.message || data.error || (res.status === 409 ? "A link with this slug already exists." : "Failed to save");
                setError(msg);
            }
        } catch (err) {
    ...
    const toggleActive = async (link: CustomLinkType) => {
        try {
    ...
    const openAnalytics = async (link: CustomLinkType) => {
    ...
    if (loading && links.length === 0) return <div className="p-8 text-center text-muted-foreground">Loading...</div>;
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto p-4 lg:p-8">
            ...
            {fetchError && (
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 p-6 rounded-xl border border-destructive/30 bg-destructive/5">
                    <AlertCircle className="h-5 w-5 text-destructive shrink-0" />
                    <p className="text-sm text-destructive">{fetchError}</p>
                    <Button variant="outline" size="sm" onClick={() => fetchLinks()} className="gap-2">
                            ...
                            <div className="mt-auto pt-4 space-y-3">
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                                    <div className="rounded-md bg-white/5 px-2 py-1.5 border border-white/5">
                                        ...
                                        <p className="font-semibold">{link.visitsCount}</p>
                                    </div>
                                    <div className="rounded-md bg-white/5 px-2 py-1.5 border border-white/5">
                                        <span className="text-muted-foreground">Reg.</span>
                                        <p className="font-semibold">{link.registrationsCount ?? 0}</p>
                                    </div>
                                    <div className="rounded-md bg-white/5 px-2 py-1.5 border border-white/5">
                                        <span className="text-muted-foreground">Purchases</span>
                                        <p className="font-semibold">{link.purchasesCount ?? 0}</p>
                                    </div>
                                    <div className="rounded-md bg-white/5 px-2 py-1.5 border border-white/5">
                                        <span className="text-muted-foreground">Revenue</span>
                                        <p className="font-semibold">{typeof link.revenue === "number" ? link.revenue.toFixed(2) : "0"} PLN</p>
                                    </div>
                                </div>
                                {link.dailyClicks && link.dailyClicks.length > 0 && (
                                    <div>
                                        <p className="text-[10px] text-muted-foreground mb-1.5 flex items-center gap-1">
                                            <TrendingUp className="h-3 w-3" /> Last 7 days
                                        </p>
                                        <div className="h-12 flex items-end gap-0.5 w-full">
                                            {link.dailyClicks.map((d, i) => {
                                                const maxCount = Math.max(...link.dailyClicks!.map((x) => x.count), 1);
                                                const heightPct = Math.max((d.count / maxCount) * 100, 4);
                                                return (
                                                    <div key={i} className="flex-1 flex flex-col justify-end items-center group" title={`${d.date}: ${d.count} clicks`}>
                                                        <div className="w-full bg-primary/30 hover:bg-primary/50 transition-colors rounded-t" style={{ height: `${heightPct}%` }} />
                                                    </div>
                                                );
                                            })}
                                        </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                            <div className="p-4 rounded-lg bg-white/5 border border-white/5">
                                                ...
                                                <p className="text-xl font-bold">{analyticsData?.registrationsCount ?? 0}</p>
                                            </div>
                                            <div className="p-4 rounded-lg bg-white/5 border border-white/5">
                                                <p className="text-xs text-muted-foreground">Purchases</p>
                                                <p className="text-xl font-bold">{analyticsData?.purchasesCount ?? 0}</p>
                                            </div>
                                            <div className="p-4 rounded-lg bg-white/5 border border-white/5">
                                                <p className="text-xs text-muted-foreground">Revenue</p>
                                                <p className="text-xl font-bold">{(analyticsData?.revenue ?? 0).toFixed(2)} PLN</p>
                                            </div>
                                        </div>
