...
export async function GET(
    ...
    const apiUrl = v ? `/public/referral/${encodeURIComponent(code)}?v=${encodeURIComponent(v)}` : `/public/referral/${encodeURIComponent(code)}`;
    ...
    try {
        ...
        console.error(`[Referral] Error resolving /r/${code}:`, error);
    }

    const protocol = request.headers.get("x-forwarded-proto") || "https";
    const host = request.headers.get("x-forwarded-host") || request.headers.get("host") || "dyskiof.net";
    const baseUrl = `${protocol}://${host}`;
    return NextResponse.redirect(`${baseUrl}/?ref=${encodeURIComponent(code)}`, 302);
}
